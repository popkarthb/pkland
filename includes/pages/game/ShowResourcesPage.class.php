	...
	function show()
	{
		...
		foreach($reslist['prod'] as $ProdID)
		{
			...
			foreach($ressIDs as $ID) 
			{
				...
				{
                    if ($Production > 0)
					    $Production	*= $config->energy_speed_produce;
                    else if ($Production < 0)
                        $Production	*= $config->energy_speed_consume;
				}
				else
				{
		...
		$basicProduction	= array(
			...
			911	=> $basicIncome[911] > 0 ?  $basicIncome[911] * $config->energy_speed_produce : $basicIncome[911] * $config->energy_speed_consume,
