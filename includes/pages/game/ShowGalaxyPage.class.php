...
class ShowGalaxyPage extends AbstractGamePage
{
	...
	public function show()
	{
        ...
        $ResultArray = array();
        for($systemCount=1; $systemCount<=$config->max_system; $systemCount++) {
            $galaxyRows	= new GalaxyRows;
            $galaxyRows->setGalaxy($galaxy);
            $galaxyRows->setSystem($systemCount);
            $ResultArray[$systemCount] = $galaxyRows->getGalaxyData();
        }
        $this->tplObj->loadscript('galaxy.js');
        $this->assign(array(
            'GalaxyRows'				=> $Result,
            'ResultArray'				=> $ResultArray,
			'planetcount'				=> sprintf($LNG['gl_populed_planets'], $Resultcount),
			'action'					=> $action,
			'galaxy'					=> $galaxy,
			'system'					=> $system,
			'planet'					=> $planet,
