...
{	
	...
	public function show()
	{
		...
		if(!empty($TheCommand) && $_SERVER['REQUEST_METHOD'] === 'POST' && $USER['urlaubs_modus'] == 0)
		{
			...
			$scrollHeight      	= HTTP::_GP('scrollHeight', 0);
			$_SESSION['scrollHeight'] = $scrollHeight;
			switch($TheCommand)
			{
				case 'cancel':
			...
			$this->redirectTo('game.php?page=buildings#'.$scrollHeight);
		}
		...
		foreach($Elements as $Element)
		{
			...
			if(in_array($Element, $reslist['prod']))
			{
                ...
                // 电力因数分开计算 modify by jeff 20231010
				if($requireEnergy < 0) {
                    $requireEnergy	= round($requireEnergy * $config->energy_speed_consume);
					$infoEnergy	= sprintf($LNG['bd_need_engine'], pretty_number(abs($requireEnergy)), $LNG['tech'][911]);
				} else {
                    $requireEnergy	= round($requireEnergy * $config->energy_speed_produce);
