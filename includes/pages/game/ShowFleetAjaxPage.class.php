	...
	public function show()
	{
		...
		switch($targetMission)
		{
			case 6:
				...
				$ships	= min($USER['spio_anz'], $PLANET[$resource[214]]);
				$shipsID = 214;
				if(empty($ships)) {
					$ships	= min($USER['spio_anz'], $PLANET[$resource[202]]);
					$shipsID = 202;
					if(empty($ships)) {
				...
				$fleetArray = array($shipsID => $ships);
				$this->returnData['ships'][$shipsID]	= $PLANET[$resource[$shipsID]] - $ships;
				break;
			case 8:
				...
				$recElementIDs	= array(401,215,207);
