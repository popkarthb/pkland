...
class FleetFunctions 
{
	...
	{	
		...
		elseif ($MissionInfo['planettype'] == 2) {
			if ((isset($MissionInfo['Ship'][207]) || isset($MissionInfo['Ship'][215]) || isset($MissionInfo['Ship'][401])) && isModuleAvailable(MODULE_MISSION_RECYCLE) && !($GetInfoPlanet['der_metal'] == 0 && $GetInfoPlanet['der_crystal'] == 0))
				$availableMissions[]	= 8;
		} else {
			if (!$UsedPlanet) {
				...
				if (!$YourPlanet && (self::OnlyShipByID($MissionInfo['Ship'], 214) || self::OnlyShipByID($MissionInfo['Ship'], 202)) && isModuleAvailable(MODULE_MISSION_SPY))
					$availableMissions[]	= 6;
				...
				if (!$YourPlanet && $MissionInfo['planettype'] == 3 && isset($MissionInfo['Ship'][220]) && isModuleAvailable(MODULE_MISSION_DESTROY))
					$availableMissions[]	= 9;
