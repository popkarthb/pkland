...
class MissionCaseExpedition extends MissionFunctions implements Mission
{
	...
	function EndStayEvent()
	{
		...
		switch($GetEvent)
		{
			...
			case 9:
			case 2:
				$eventSize   = mt_rand(0, $fleetPoints);
                $Size       = 10;

				if($eventSize<=50000) {
					$Size		= mt_rand(100, 1000)*log10($fleetPoints);
					$Message	= $LNG['sys_expe_found_dm_1_'.mt_rand(1,5)];
				}
				elseif(50000<$eventSize && $eventSize <= 1000000) {
					$Size		= mt_rand(1000, 5000)*log10($fleetPoints);;
					$Message	= $LNG['sys_expe_found_dm_2_'.mt_rand(1,3)];
				} 
				else{
					$Size	 	= mt_rand(5000, 9000)*log10($fleetPoints);
			...
			case 3:
				...
				if(10 < $eventSize) {
					$Size		= mt_rand(10, 50)*log10($fleetPoints);
					$Message	= $LNG['sys_expe_found_ships_1_'.mt_rand(1,4)];
				} elseif(0 < $eventSize && 10 >= $eventSize) {
					$Size		= mt_rand(52, 100)*log10($fleetPoints);
					$Message	= $LNG['sys_expe_found_ships_2_'.mt_rand(1,2)];
				} elseif(0 == $eventSize) {
					$Size	 	= mt_rand(102, 200)*log10($fleetPoints);
