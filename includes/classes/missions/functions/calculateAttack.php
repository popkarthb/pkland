	...
	for ($ROUNDC = 0; $ROUNDC <= MAX_ATTACK_ROUNDS; $ROUNDC++) 
	{
		...
		foreach ($attackers as $fleetID => $attacker) {
			...
			foreach($attacker['unit'] as $element => $amount) {
				...
				if ($attArray[$fleetID][$element]['shield'] == 0) {
					echo '<pre>';
					var_dump("defArray", $attArray);
					echo '</pre>';
					echo "fleetID: " . $fleetID . "<br>";
					echo "element: " . $element . "<br>";
					
					$ile_removePoints = max(min($max_removePoints, $amount * 1), 0);
					}
				else
					$ile_removePoints = max(min($max_removePoints, $amount * min($defender_moc / $attArray[$fleetID][$element]['shield'] * (rand(1, 1) / 100), 1)), 0);
		...
		foreach ($defenders as $fleetID => $defender) {
			...
			foreach($defender['unit'] as $element => $amount) {
				...
				if ($defArray[$fleetID][$element]['shield'] == 0) {
					echo '<pre>';
					var_dump("defArray", $defArray);
					echo '</pre>';
					echo "fleetID: " . $fleetID . "<br>";
					echo "element: " . $element . "<br>";
					...
					$ile_removePoints = max(min($max_removePoints, $amount * min($attacker_moc / $defArray[$fleetID][$element]['shield'] * (rand(1, 1) / 100), 1)), 0);
