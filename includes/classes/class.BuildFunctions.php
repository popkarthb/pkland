...
class BuildFunctions
{
        ...
        foreach ($reslist['ressources'] as $resType)
        {
			...
			if (in_array($Element, $reslist['build']) || in_array($Element, $reslist['tech']) )
				...
				if(isset($pricelist[$Element]['factor']) && $pricelist[$Element]['factor'] != 0 && $pricelist[$Element]['factor'] != 1) {
					$exp_price	    = $ressourceAmount * pow($pricelist[$Element]['factor'], $elementLevel);
					$linear_price	= 88888 * $ressourceAmount * $pricelist[$Element]['factor'] * $elementLevel + ($ressourceAmount * pow($pricelist[$Element]['factor'], $elementLevel))%88888;
					
					if ($exp_price<$linear_price)
						$price[$resType] = $exp_price;
					else
						$price[$resType] = $linear_price;
					
					if ($price[$resType] > 999999999) $price[$resType] = 999999999;
					
					
					//$price[$resType]	= $price[$resType] * pow($pricelist[$Element]['factor'], $elementLevel);
					
				}
			
			if(in_array($Element, $reslist['fleet']) || in_array($Element, $reslist['defense']) || in_array($Element, $reslist['missile'])  ){
                $price[$resType]	*= $pricelist[$Element]['factor'];
            }
    ...
    public static function getBuildingTime($USER, $PLANET, $Element, $elementPrice = NULL, $forDestroy = false, $forLevel = NULL)
    {
        ...
        $time   = 1/60; //unit 1 hour;  1/60 = 1 min 
        ...
        if	   (in_array($Element, $reslist['build'])) {
            $time	= 1/60 * $elementCost / ($config->game_speed) * (1 + $USER['factor']['BuildTime']) * pow(0.95, $PLANET[$resource[15]]) ;
			
        } elseif (in_array($Element, $reslist['fleet'])) {

			if 		(in_array($Element, [202,203,204,205,206]))
				$time	= 1/60 * $elementCost / ($config->game_speed) * (1 + $USER['factor']['ShipTime']) * pow(0.97, $PLANET[$resource[14]]) * pow(0.95, $PLANET[$resource[21]]);
			elseif  (in_array($Element, [207,208,209,210,211,212,213]))
				$time	= 1/60 * $elementCost / ($config->game_speed) * (1 + $USER['factor']['ShipTime']) * pow(0.97, $PLANET[$resource[14]]) * pow(0.95, $PLANET[$resource[41]]);
			elseif  (in_array($Element, [214,215,216,217,218,219,220]))
				$time	= 1/60 * $elementCost / ($config->game_speed) * (1 + $USER['factor']['ShipTime']) * pow(0.97, $PLANET[$resource[14]]) * pow(0.95, $PLANET[$resource[43]]);
			elseif  (in_array($Element, [401,402,403,404,405,406]))
				$time	= 1/60 * $elementCost / ($config->game_speed) * (1 + $USER['factor']['ShipTime']) * pow(0.97, $PLANET[$resource[14]]) * pow(0.95, $PLANET[$resource[42]]);
			else
				$time	= 1/60 * $elementCost / ($config->game_speed) * (1 + $USER['factor']['ShipTime']) * pow(0.97, $PLANET[$resource[14]]);
			
			
        } elseif (in_array($Element, $reslist['defense'])) {
            $time	= 1/60 * $elementCost / ($config->game_speed) * (1 + $USER['factor']['DefensiveTime']) * pow(0.97, $PLANET[$resource[14]]);
			...
			$time	= 1/60 * $elementCost / ($config->game_speed) * (1 + $USER['factor']['ResearchTime']) * pow(0.95, $PLANET[$resource[31]]) * pow(0.90, $PLANET[$resource[6]]);
			
			

            //$time	= $elementCost * 10 / (1000 * (1 + $Level)) / ($config->game_speed / 2500) * pow(1 - $config->factor_university / 100, $PLANET[$resource[6]]) * (1 + $USER['factor']['ResearchTime']);
			
			
			
			
			
			//if ($time > 168) $time = 168 + $elementCost / (3600 * (1 + $Level)) / ($config->game_speed / 2500);
