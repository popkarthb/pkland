...
<div id="leftmenu">
	<ul id="menu">
		...
		{if allowedTo('ShowSpeedUpPage')}<li><a href="?page=speedup" target="Hauptframe">{$LNG.mu_speed_up}</a></li>{/if}
		...
		{if allowedTo('ShowCreatorIntruderBasePage')}<li><a href="?page=createintruderbase" target="Hauptframe">{$LNG.new_creator_intruder_base}</a></li>{/if}
