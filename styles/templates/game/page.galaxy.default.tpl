...
{block name="content"}
	...
	<table class="table569">
		<tr>
			<th colspan="2">{$LNG.gl_missil_launch} [<input name="galaxy" value="{$galaxy}"style="width: 40px;">:<input name="system" value="{$system}" style="width: 20px;">:<input name="planet" value="{$planet}" style="width: 20px;">]</th>
		</tr>
		<tr>
	...
	<style>
		.table-with-same-width-height .custom-20width {
		width: 20px; 
		white-space: nowrap;
		}
		.custom-20width {
		...
		.table-with-same-width-height .custom-20height {
		height: 20px; /* Set the desired width */
		white-space: nowrap; /* Preserve white space */
		}
		
		.table-with-same-width-height td {
		width: 111px; /* Replace with your desired width */
		height: 111px; /* Replace with your desired height */
		text-align: center;
		}
		
		.table-with-same-width-height .blue-bg {
		  background-color: rgb(71 82 237 / 80%) !important;
		}
		
		.table-with-same-width-height .green-bg {
		  background-color: rgb(16 131 10 / 80%) !important;
		}

	</style>
	
	<table class="table-with-same-width-height">
    <tr>
		<th colspan="11">{$LNG.gl_solar_system} {$galaxy}</th>
	</tr>
	<tr>
		<th class="custom-20width">
		...
		<th style="white-space: nowrap"class="center-text">1</th>
		<th style="white-space: nowrap"class="center-text">2</th>
		<th style="white-space: nowrap"class="center-text">3</th>
		<th style="white-space: nowrap"class="center-text">4</th>
		<th style="white-space: nowrap"class="center-text">5</th>
		<th style="white-space: nowrap"class="center-text">6</th>
		<th style="white-space: nowrap"class="center-text">7</th>
		<th style="white-space: nowrap"class="center-text">8</th>
		<th style="white-space: nowrap"class="center-text">9</th>
		<th style="white-space: nowrap"class="center-text">10</th>
		<!--
	...
	{for $system=1 to $max_system}  
	<tr>
		<th class="custom-20width">{$system}</th>
	    {for $planet=1 to $max_planets}
		
		
		{if $system==1 || $system==9 || $planet==1 || $planet==9}
			<td class="blue-bg">
		{else}
			<td class="green-bg">
		{/if}
		
	    {if !isset($ResultArray[$system][$planet])}
			<a href="?page=fleetTable&amp;galaxy={$galaxy}&amp;system={$system}&amp;planet={$planet}&amp;planettype=1&amp;target_mission=7">{$system}:{$planet}</a><br><br><br><br><br><br><br>
	    {elseif $ResultArray[$system][$planet] === false}
			{$system}:{$planet}<br>{$LNG.gl_planet_destroyed}
	    {else}
			{$system}:{$planet}
	        {$currentPlanet = $ResultArray[$system][$planet]}
			<br>
				...
				<img src="{$dpath}planeten/small/s_{$currentPlanet.planet.image}.jpg" height="30" width="30" alt=""> </a>
				...
				<img src="{$dpath}planeten/small/s_mond.jpg" height="22" width="22" alt="{$currentPlanet.moon.name}"></a>
				{/if}
	...
	<!--
	<tr>
		<td class="custom-20height">{$max_planets + 1}</td>
		<td class="custom-20height" colspan="10"><a href="?page=fleetTable&amp;galaxy={$galaxy}&amp;system={$system-1}&amp;planet={$max_planets + 1}&amp;planettype=1&amp;target_mission=15">{$LNG.gl_out_space}</a></td>
	</tr>
	-->
	...
	<table>
	<tr>
		<td class="custom-20height" colspan="1">({$planetcount})</td>
		<td class="custom-20height" colspan="1"><span id="missiles">{$currentmip|number}</span> {$LNG.gl_avaible_missiles}</td>
		<td class="custom-20height" colspan="1"><span id="slots">{$maxfleetcount}</span>/{$fleetmax} {$LNG.gl_fleets}</td>
		<td class="custom-20height" colspan="1">
