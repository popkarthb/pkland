	...
	<table style="width:760px">
		...
		<tr>
			<td style="width:70%;vertical-align:top;" class="left">
				{$List@iteration}.: 
				{if !($isBusy.research && ($ID == 6 || $ID == 31)) && !($isBusy.shipyard && ($ID == 14 || $ID == 21 ||$ID == 41 ||$ID == 42 || $ID == 43 )) && $RoomIsOk && $CanBuildElement && $BuildInfoList[$ID].buyable}
			...
			<td>
				...
				<form action="game.php?page=buildings" method="post" class="build_form">
					...
					<button type="buttom" onclick="return Dialog.open('game.php?page=speedup&from=building', 523, 223)" class="build_submit onlist">{$LNG.bd_speed_up}</button>
			...
			<td>
				...
				{/if}
				<br><span style="color:green" data-time="{$List.endtime}" class="timer">{$List.display}</span>
			</td>
		</tr>
	{/foreach}
	</table>
		...
		<td>
			<table style="width:100%" id="{$ID}">
				<tr>
					...
					{elseif ($isBusy.research && ($ID == 6 || $ID == 31)) || ($isBusy.shipyard && ($ID == 14 || $ID == 21 ||$ID == 41 ||$ID == 42 || $ID == 43))}
							...
							{if ($ID == 44 && !$HaveMissiles) ||  $ID != 44}<br><a class="tooltip_sticky" data-tooltip-content="
								...
								<table style='width:300px'>
									...
									<tr>
										...
										<td><span style='color:{if empty($Element.destroyOverflow[$RessID])}green{else}red{/if}'>{$ResCount|number}</span></td>
									</tr>
									{/foreach}
									<tr>
