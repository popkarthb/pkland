...
<table style="width:760px">
	<tr>
		<td class="transparent">
			...
			<tr>
				<td>
					<select name="auftr[]" id="auftr" size="10" multiple><option>&nbsp;</option></select><br><br>{$LNG.bd_cancel_warning}<br>
					<input type="button" onclick="speedup();" value="{$LNG.bd_speed_up}">&nbsp;&nbsp;
...
function speedup(){
	var selectedValues = $('#auftr option:selected').map(function() {
		return $(this).val();
	}).get();
	console.log(selectedValues.join())

	Dialog.open('game.php?page=shipyardspeedup&from=shipyard&auftr='+ selectedValues.join(), 523, 223);
}
</script>
{/if}
{/block}
